{
  gROOT->Reset();
  const char * errors_fromresolution="DATA/bkg3results_from_resolution.dat";
  const char * bkg3randata="DATA/bkg3randomdata.dat"; //storing the mean and sigma from toy mc
  const char * saving_random="PLOT/bkg3randompd.pdf";
  ...
  if(inputfromran.is_open())
	...
	ofstream ranoutput(bkg3randata);
	if(ranoutput.is_open())
	  {
	    ...
	    cout<<"successfully stored random resolution mc data into file "<<bkg3randata<<endl;
	
	  }
	else
	  {
	    cout<<"Unable to open file "<<bkg3randata<<endl;
	    return 0;
	  }
