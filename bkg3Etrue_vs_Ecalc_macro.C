{
  const char * filedata="DATA/bkg3E_estandE_true.dat"; //File storing the estimated and true energy and their errors
  const char * rootfile="ROOTFILES/bkg3true_estimated_enregyplot.root"; //root file to save the plot
  const char * filenamee="DATA/bkg3slope_intercept.dat"; //file to store slope and intercept of plot
  const char * pdfetrue="PLOTS/bkg3EtrueECalc.pdf"; //pdf file location
  c=new TCanvas();
  c->SetGrid();
  c->SetFillColor(42);
